...
load("integrated_top2000genes.Rdata")
...
expression<-FindIntegrationAnchors(object.list =expression, dims = 1:30)
...
#save(expression_inter,experiment,file="integrated_top2000genes.Rdata")
...
####################visualise tsne #######################
##########################################################
tiff(file = "Cattle_Human_TSNE1_all_gene.tiff",##color by species
...
g<-ggplot(info.norm, aes(x = tsne1, y = tsne2)) + 
  geom_point(alpha = 1,size=3,aes(x = tsne1, y = tsne2, colour = experiment$species)) + theme_bw()+
  xlab("t-SNE-1")+ylab("t-SNE-2")+
  theme(panel.background=element_blank(),legend.title=element_blank(),legend.text = element_text(size=23),
  ...
  scale_colour_manual(name="Tissue_categories",values=colors)
...
tiff(file = "Cattle_Human_TSNE2_all_gene.tiff",##reqiured to change
     res = 300, width = 3000, height = 2150,compression = "lzw")
par(mar=c(2,2,5,5))
info.norm<-as.data.frame(expression_tsne$Y)
colnames(info.norm)<-c("tsne1","tsne2")
colors<-c("#00008B","#008B8B","#8B008B","#8B0000","#828282","#00B2EE","#0000FF","#FF0000","#FF00FF","#FF8C00","#FFFF00","#006400","#00FF00",
...
ggplot(info.norm, aes(x = tsne1, y = tsne2, colour = experiment$tissue_new)) + 
  geom_point(alpha = 0.8,size=3) + theme_bw()+
  xlab("t-SNE-1")+ylab("t-SNE-2")+
  theme(panel.background=element_blank(),legend.title=element_blank(),legend.text = element_text(size=25),
        panel.grid.minor=element_blank(),panel.grid.major=element_blank(),
        axis.title.y=element_text(size=25,color="black"),
        axis.title.x=element_text(size=25,color="black"),
        axis.text.y=element_text(size=20,color="black"),
        axis.text.x=element_text(size=20,color="black",hjust=0.95,vjust=0.2))+
  scale_colour_manual(name="Tissue_categories",values=colors)+guides(color = guide_legend(override.aes = list(size=5),ncol=1))
#+stat_ellipse(type="norm")
dev.off()
