...
variance_tissue_tpm<-aggregate(expression,list(as.factor(experiment$annotation)),var)
variance_tissue_tpm
...
tiff(file = "variance_sorted_new.tiff",##reqiured to change
...
title(ylab="Percentage of shared orthologs", xlab = "Windows of genes sorted by expression variance",line=4, cex.lab=2.5)
...
col<-c("#00008B","#008B8B","#8B008B","#8B0000","#828282","#00B2EE","#0000FF","#FF0000","#FF00FF","#FF8C00","#FFFF00","#006400","#00FF00",
       "#CD950C","#FF6A6A","#CCCC33","pink","#FF1493","#8B7355","#00F5FF")
tiff(file = "mad_sorted.tiff",##reqiured to change
     res = 300, width = 5200, height = 2000,compression = "lzw")
par(mar=c(8,8,6,1))
my<-barplot(Results1_percent,beside = T,col=col,cex.axis = 1.8,cex.names = 1.8,ylim = c(0,0.7))
